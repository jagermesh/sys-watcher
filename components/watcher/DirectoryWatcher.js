        ...
        walk(path, match, except, filters, function(error, results) {
          if (error) {
            ...
            if ((results.length > 0) || _this.config.settings.reportEmpty) {
              ...
              // }
              let details = { Path: path, Files: results };
              let message;
              ...
              _this.getApplication().notify(_this.getLoggers(), { message: message, value: results.length, units: 'Count', dimensions: { Path: path } }, details, _this);
            }
          }
        });
      }
