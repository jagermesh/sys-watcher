...
function RabbitMQWatcher(application, name, config) {
  ...
  function watchRule(amqpConnection, queueName, queueConfig) {
    ...
    queueConfig.exchangeSettings  = queueConfig.exchangeSettings  || { durable: true };
    queueConfig.queueName         = queueConfig.queueName         || '';
    queueConfig.queueSettings     = queueConfig.queueSettings     || { exclusive: true, durable: true, auto_delete: true };
    queueConfig.consumingSettings = queueConfig.consumingSettings || { noAck: true };
    queueConfig.bindingKey        = queueConfig.bindingKey        || '';

    queueConfig.queueName         = queueConfig.queueName.replace('{ip}', ip.address());

    let details = { Rule: queueName, Exchange: queueConfig.exchangeName, Queue: queueConfig.queueName };

    amqpConnection.createChannel(function(error, channel) {
      if (error) {
        _this.getApplication().notify(_this.getLoggers(), { message: 'Can not create channel: ' + error.toString(), isError: true}, details, _this);
      } else {
        channel.assertExchange(queueConfig.exchangeName, queueConfig.exchangeType, queueConfig.exchangeSettings, function(error, exchange) {
          if (error) {
            _this.getApplication().notify(_this.getLoggers(), { message: 'Can not create exchange: ' + error.toString(), isError: true}, details, _this);
          } else {
            channel.assertQueue(queueConfig.queueName, queueConfig.queueSettings, function(error, queue) {
              if (error) {
                _this.getApplication().notify(_this.getLoggers(), { message: 'Can not create queue: ' + error.toString(), isError: true}, details, _this);
              } else {
                channel.bindQueue(queue.queue, queueConfig.exchangeName, queueConfig.bindingKey, null, function(error, binding) {
                  if (error) {
                    _this.getApplication().notify(_this.getLoggers(), { message: 'Can not create binding: ' + error.toString(), isError: true }, details, _this);
                  } else {
  ...
  function watch(ruleName, ruleConfig) {
    ...
    let details = { Rule: ruleName };

    amqpcb.connect(_this.config.settings.connectString, function(error, connection) {
      if (error) {
        _this.getApplication().notify(_this.getLoggers(), { message: 'Can not connect to RabbitMQ server, retrying in 30 seconds: ' + error.toString(), isError: true }, details, _this);
        setTimeout(function() {
          watch(ruleName, ruleConfig);
        }, 30000);
      } else {
        connection.on('error', function(error) {
          _this.getApplication().notify(_this.getLoggers(), { message: 'Error during communication with RabbitMQ server: ' + error.toString(), isError: true }, details, _this);
        });
        connection.on('close', function(error) {
          _this.getApplication().notify(_this.getLoggers(), { message: 'Lost connection with RabbitMQ server, reconnecting in 30 seconds: ' + error.toString(), isError: true }, details, _this);
          setTimeout(function() {
            watch(ruleName, ruleConfig);
          }, 30000);
        });
