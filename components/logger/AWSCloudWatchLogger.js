      ...
      if (data && (data.value != undefined)) {
        ...
        if (data.dimensions) {
          for(let name in data.dimensions) {
            dimensions.push({ Name: name
                            , Value: data.dimensions[name]
                            });
          }
        }

        let params = { MetricData: [
        ...
        let cloudwatch = new aws.CloudWatch({ region: _this.config.settings.AWS.region
